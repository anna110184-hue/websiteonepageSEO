// 更新後的 daily-card Edge Function - 匹配實際 Tarot_card_meaning 表格結構
...
async function handleDrawDailyCard(supabase: any, userId: string, body: any) {
  ...
  if (existing) throw new Error('Daily card already drawn today')
...
async function handleUpdateDailyCard(supabase: any, userId: string, body: any) {
  ...
  if (body.personal_reflection !== undefined) updates.personal_reflection = body.personal_reflection
  if (body.mood_after !== undefined) updates.mood_after = body.mood_after
  if (body.key_insights !== undefined) updates.key_insights = body.key_insights
  if (body.accuracy_rating !== undefined) updates.accuracy_rating = body.accuracy_rating
  if (body.helpfulness_rating !== undefined) updates.helpfulness_rating = body.helpfulness_rating
  if (body.learned_something_new !== undefined) updates.learned_something_new = body.learned_something_new
  if (body.new_learning_notes !== undefined) updates.new_learning_notes = body.new_learning_notes
  if (body.synchronicity_notes !== undefined) updates.synchronicity_notes = body.synchronicity_notes
  if (body.is_shared !== undefined) updates.is_shared = body.is_shared
  if (body.share_message !== undefined) updates.share_message = body.share_message
